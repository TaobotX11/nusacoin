...
class WalletTest(NusacoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 NUX from 0 to 2 using sendtoaddress call.
        ...
        assert_equal(self.nodes[0].getbalance(), Decimal('10'))
         ...
         # Sendmany with explicit fee (NUX/kB)
        ...
        # Sendmany with explicit fee (NU/B)
        # Throw if no conf_target provided
        assert_raises_rpc_error(-8, "Selected estimate_mode requires a fee rate",
            self.nodes[2].sendmany,
            amounts={ address: 10 },
        ...
        # Send 10 NUX with subtract fee from amount
        ...
        # Sendmany 10 NUX with subtract fee from amount
        ...
        # This will raise an exception for attempting to get the private key of an invalid Nusacoin address
        assert_raises_rpc_error(-5, "Invalid Nusacoin address", self.nodes[0].dumpprivkey, "invalid")

        # This will raise an exception for attempting to set a label for an invalid Nusacoin address
        assert_raises_rpc_error(-5, "Invalid Nusacoin address", self.nodes[0].setlabel, "invalid address", "label")

        # This will raise an exception for importing an invalid address
        assert_raises_rpc_error(-5, "Invalid Nusacoin address or script", self.nodes[0].importaddress, "invalid")
        ...
        # send with explicit nux/kb fee
            self.log.info("test explicit fee (sendtoaddress as nux/kb)")
            self.nodes[0].generate(1)
            self.sync_all(self.nodes[0:3])
