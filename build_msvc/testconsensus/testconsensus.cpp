...
int main()
{
    std::cout << "nusacoinconsensus version: " << nusacoinconsensus_version() << std::endl;
    ...
    auto op0Result = nusacoinconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, nusacoinconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
