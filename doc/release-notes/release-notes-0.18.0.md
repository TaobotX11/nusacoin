Nusacoin Core version 0.18.0 is now available from:

  <https://nusacoincore.org/bin/nusacoin-core-0.18.0/>
...
`/Applications/Nusacoin-Qt` (on Mac) or `nusacoind`/`nusacoin-qt` (on
Linux).
...
Nusacoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Nusacoin Core on unsupported systems.
...
10.10. Additionally, Nusacoin Core does not yet change appearance when
...
`contrib/init/nusacoind.service`, it has been changed to use
`/var/lib/nusacoind` as the data directory instead of
`~nusacoin/.nusacoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/nusacoind` will
exist has enough space (check using `df -h /var/lib/nusacoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/nusacoin/nusacoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://nusacoincore.org/en/contact/) the Nusacoin Core
...
The systemd init file (`contrib/init/nusacoind.service`) has been changed
to use `/var/lib/nusacoind` as the data directory instead of
`~nusacoin/.nusacoin`. This change makes Nusacoin Core more consistent with
...
`datadir` option in `/etc/nusacoin/nusacoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/nusacoin/nusacoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/nusacoin/nusacoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/nusacoin/nusacoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/nusacoin/nusacoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/nusacoin/nusacoin/blob/master/doc/nusacoin-conf.md)
  about the `nusacoin.conf` file describes how to use it to configure
  Nusacoin Core.

- A new document introduces Nusacoin Core's BIP174 [Partially-Signed
  Nusacoin Transactions
  (PSBT)](https://github.com/nusacoin/nusacoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/nusacoin/nusacoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Nusacoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Nusacoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/nusacoin/nusacoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/nusacoin/nusacoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Nusacoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Nusacoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/nusacoin/nusacoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/nusacoin/nusacoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart nusacoind with the `-deprecatedrpc=generate`
...
- A new `nusacoin-wallet` tool is now distributed alongside Nusacoin
...
This section describes planned changes to Nusacoin Core that may affect
other Nusacoin software and services.

- Since version 0.16.0, Nusacoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Nusacoin Core 0.20 (expected about
  a year after 0.18), Nusacoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Nusacoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Nusacoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Nusacoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Nusacoin Core
...
- A sub-project of Nusacoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Nusacoin Core.  See their [project
  page](https://github.com/nusacoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Nusacoin Core's own implementation, although entropy
  gathered by Nusacoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Nusacoin Core a
...
- On macOS, Nusacoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `nusacoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libnusacoin_server.a` (jnewbery)
...
- #13926 [Tools] nusacoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like nusacoind (MarcoFalke)
...
- #14521 qt, docs: Fix `nusacoin-qt -version` output formatting (hebasto)
...
- #11625 Add NusacoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libnusacoin_server` (laanwj)
...
- #15154 Configure: nusacoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/nusacoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Nusacoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libnusacoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in nusacoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Nusacoin Core (Christewart)
...
- #12246 Bugfix: Only run nusacoin-tx tests when nusacoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_nusacoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_nusacoin` (MarcoFalke)
...
- #14186 nusacoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_nusacoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update nusacoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/nusacoin-conf.md (hebasto)
...
- #15354 Add missing `nusacoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct nusacoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support nusacoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/nusacoin/).
