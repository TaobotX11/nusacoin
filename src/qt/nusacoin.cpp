...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. nusacoin_de.qm (shortcut "de" needs to be defined in nusacoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. nusacoin_de_DE.qm (shortcut "de_DE" needs to be defined in nusacoin.qrc)
...
NusacoinCore::NusacoinCore(interfaces::Node& node) :
    QObject(), m_node(node)
{
}

void NusacoinCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(m_node.getWarnings()));
}

void NusacoinCore::initialize()
{
    try
    {
...
void NusacoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the NusacoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", NusacoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void NusacoinApplication::createPaymentServer()
{
...
void NusacoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    ...
    connect(pollShutdownTimer, &QTimer::timeout, window, &NusacoinGUI::detectShutdown);
}
...
void NusacoinApplication::startThread()
{
    ...
    connect(executor, &NusacoinCore::initializeResult, this, &NusacoinApplication::initializeResult);
    connect(executor, &NusacoinCore::shutdownResult, this, &NusacoinApplication::shutdownResult);
    connect(executor, &NusacoinCore::runawayException, this, &NusacoinApplication::handleRunawayException);
    connect(this, &NusacoinApplication::requestedInitialize, executor, &NusacoinCore::initialize);
    connect(this, &NusacoinApplication::requestedShutdown, executor, &NusacoinCore::shutdown);
...
void NusacoinApplication::parameterSetup()
{
...
void NusacoinApplication::requestInitialize()
{
...
void NusacoinApplication::requestShutdown()
{
...
void NusacoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        // nusacoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NusacoinGUI::handlePaymentRequest);
            connect(window, &NusacoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void NusacoinApplication::shutdownResult()
{
    quit(); // Exit second main loop invocation after shutdown finished
}

void NusacoinApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(nullptr, "Runaway exception", NusacoinGUI::tr("A fatal error occurred. Nusacoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId NusacoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", NusacoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(nusacoin);
    Q_INIT_RESOURCE(nusacoin_locale);
    ...
    // nusacoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
