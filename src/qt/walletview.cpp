    ...
    platformStyle(_platformStyle)
{
    ...
    // Pass through event to overview
    connect(this, &WalletView::setPrivacy, overviewPage, &OverviewPage::setPrivacy);
    connect(this, &WalletView::setPrivacy, sendCoinsPage, &SendCoinsDialog::setPrivacy);
...
void WalletView::setNusacoinGUI(NusacoinGUI *gui)
{
    if (gui)
    {
        // Clicking on a transaction on the overview page simply sends you to transaction history page
        connect(overviewPage, &OverviewPage::transactionClicked, gui, &NusacoinGUI::gotoHistoryPage);

        // Navigate to transaction history page after send
        connect(sendCoinsPage, &SendCoinsDialog::coinsSent, gui, &NusacoinGUI::gotoHistoryPage);
        ...
        connect(this, &WalletView::encryptionStatusChanged, gui, &NusacoinGUI::updateWalletStatus);

        // Pass through transaction notifications
        connect(this, &WalletView::incomingTransaction, gui, &NusacoinGUI::incomingTransaction);

        // Connect HD enabled state signal
        connect(this, &WalletView::hdEnabledStatusChanged, gui, &NusacoinGUI::updateWalletStatus);
    }
}
...
void WalletView::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
        ...
        progressDialog->setStyleSheet("QProgressDialog { background-color: lightgray; border: 1px solid gray; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #00ff00, stop: 1 #018f01); border-radius: 4px; margin: 0px; }");
