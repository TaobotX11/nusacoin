...
static bool AppInit(int argc, char* argv[])
{
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  nusacoind [options]                     Start " PACKAGE_NAME "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see nusacoind -h for a list of options.\n", argv[i])));
            }
        }
...
int main(int argc, char* argv[])
{
    ...
    // Connect nusacoind signal handlers
    noui_connect();
